...
int main()
{
    ...
    std::string DateTime = newSat1.getIssueDateTime();
    std::cout << "2024" << std::endl;
    std::cout << DateTime << std::endl;
    SatVector.push_back(newSat1);
    AllSats.addSatellite(newSat1);
    //AllSats->addSatellite(newSat2);
    //AllSats->addSatellite(newSat3);
    //std::vector<std::shared_ptr<Satellites>> SatVector = AllSats->AllSatellites;
    std::cout << SatVector[0].getIssueDateTime() << std::endl;
    std::cout << AllSats.getSingleSatObs("2024").getIssueDateTime()<< std::endl;
}